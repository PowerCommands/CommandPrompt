...
public class TemplatePaths
{
    public TemplatePaths(string sourceModulePath, string outputPath, string projectName)
    {
        ...
        CoreLibRoot = new TemplatePath(Source: coreLibRoot, Target: Path.Combine(outputPath, "PainKiller.CommandPrompt.CoreLib"));
        ApplicationRoot = new TemplatePath(Source: promptKitRoot, Target: Path.Combine(outputPath, projectName));
